...
export interface ControlWorkerClient {
  ...
  interruptTurn(conversationId: string, turnId: string, input: { clientRequestId: string; expectedTurnId: string }): Promise<CommandAccepted>;
...
export async function submitInterrupt(options: BaseOptions & { turnId: string | null }): Promise<ControlSubmitResult> {
  if (!options.conversationId || !options.turnId) {
  ...
  try {
    await options.workerClient.interruptTurn(options.conversationId, options.turnId, {
...
export async function submitSteer(options: BaseOptions & { message: string; turnId: string | null }): Promise<ControlSubmitResult> {
  ...
  options.setStatus("submitting");
  try {
    await options.workerClient.steerTurn(options.conversationId, options.turnId, {
      clientRequestId: options.createClientRequestId(),
      expectedTurnId: options.turnId,
...
export async function submitApprovalDecision(
  ...
  if (!options.conversationId || options.approval.conversationId !== options.conversationId) {
    options.setStatus("failed");
    return "failed";
  }
  ...
  options.setStatus("submitting");
  try {
    await options.workerClient.decideApproval(options.conversationId, options.approval.id, {
      clientRequestId: options.createClientRequestId(),
