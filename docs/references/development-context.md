...
- Start, timeline, follow-up, approval pending list, and interrupt now record `real-pass`; interrupt uses an independent safe interrupt-only active-turn sample before calling the public route.
- Real Google Chrome verification found that app-server-derived conversation `name` / `preview` could expose calibration prompt text through Web conversation metadata. Worker projection now hides those fields publicly by using the allowed project basename fallback title and an empty summary.
- Documented safety gap: approval decision lacks a safe pending approval in the current real project stack. Stage 10 added an isolated fixture, but current real runs still record `approval_fixture_no_pending_request` after read-only/on-request, stdout-only command probing, and explicit `approvalsReviewer: "user"`; approval decision remains excluded from product-ready claims. Automatic accept, persistent policy amendment, user-layer rules edits, auth-copying paths, and production approval safety model are out of scope.
