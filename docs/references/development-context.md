...
- Documented safety gap: approval decision lacks a safe pending approval in the current real project stack. Stage 10 added an isolated fixture, but current real runs still record `approval_fixture_no_pending_request`; approval decision remains excluded from product-ready claims. Automatic accept, persistent policy amendment, and production approval safety model are out of scope.
