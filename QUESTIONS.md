...
- Status: Partial. Approval pending list records `real-pass`; Stage 10 isolated fixture currently records `approval decision` as `real-gap` with `reasonCode=approval_fixture_no_pending_request`. Automatic accept, persistent policy amendment, and production approval safety model remain out of scope.
